...
for listOfMatchingChars in listOfListsOfMatchingChars:
    ...
    for i in range(0, len(plates_list)):
        ...
        #print(p2fRectPoints[0], p2fRectPoints[1], p2fRectPoints[2], p2fRectPoints[3])
        #crop_img = img[int(p2fRectPoints[0][0]):int(p2fRectPoints[0][0])+int(p2fRectPoints[3][0]), int(p2fRectPoints[0][1]):int(p2fRectPoints[0][1])+int(p2fRectPoints[3][1])]
        t_out = thresh_out[int(p2fRectPoints[2][1])-10:int(p2fRectPoints[3][1])+10, int(p2fRectPoints[0][0])-10:int(p2fRectPoints[3][0])]
        cv2.imwrite("bw.jpg", t_out)

        crop_img = img[int(p2fRectPoints[2][1])-10:int(p2fRectPoints[3][1])+10, int(p2fRectPoints[0][0])-10:int(p2fRectPoints[3][0])]
        cv2.imwrite("marked_"+args["image"], img)
        cv2.imwrite("cropped_"+args["image"], crop_img)
